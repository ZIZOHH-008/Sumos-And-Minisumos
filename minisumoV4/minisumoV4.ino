...
void loop() {
  ...
  bool rivalAtrasDerecha = rivalAtras && rivalDerecha;
  bool rivalAtrasAdelante = rivalAtras && rivalAdelante;
  bool rivalDerechaAdelante = rivalDerecha && rivalAdelante;
  bool rivalTodos = rivalAtras && rivalDerecha && rivalAdelante;
  ...
  else if (rivalAtrasAdelante) {
      avanzar(255); 
  }

  else if (rivalAtrasDerecha) {
      girarDerecha(180);
  }
...
// Sin el driver, no se mueven los motores
void activarDriver() {
  digitalWrite(STBY, HIGH);
}
